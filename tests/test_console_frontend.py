...
class ConsoleFrontendTests(unittest.TestCase):
    def test_console_assets_exist_and_reference_guarded_routes(self) -> None:
        ...
        self.assertIn("responsePayload", app)
        self.assertIn("invalidJsonResponse", app)
        self.assertIn("response.text()", app)
        self.assertIn('top: 50%', styles := (CONSOLE / "styles.css").read_text(encoding="utf-8"))
        self.assertIn("closeDetailsButton", app)
        self.assertIn("clearDetails", app)
        self.assertIn("form-status", styles)
        ...
        self.assertIn("/console/monitor/active/", app)
        ...
        self.assertIn("costRowsBody", index)
        self.assertIn('/console/data/${section}', app)
        self.assertIn('data-resolution-action="disregard"', app)
        self.assertIn("Manual Override", app)
        self.assertIn("batchExceptionToolbar", index)
        self.assertIn("monitorBatchExceptionToolbar", index)
        self.assertIn('data-exception-select=', app)
        self.assertIn("resolveBatchExceptionForm", app)
        self.assertIn('"/console/exceptions/resolve-batch"', app)
        self.assertIn("batch-select-all-exceptions", app)
        self.assertIn("Prepare", app)
        self.assertIn("Finish", app)
        self.assertIn('data-resolution-action="manualCategory"', app)
        self.assertIn('data-resolution-action="moveEmail"', app)
        self.assertIn("exceptionOverviewTable", app)
        ...
        self.assertIn("artifactBytesFromDownload", app)
        self.assertIn("table-link-button", styles)
        self.assertIn("processedOrdersPager", index)
        self.assertIn("webstoreOrdersPager", index)
        self.assertIn("nonOrderEmailsPager", index)
        self.assertIn("MONITOR_PROCESSED_PAGE_SIZE = 20", app)
        self.assertIn('data-action="monitor-page"', app)
        self.assertNotIn('name="forceMove"', app)
        self.assertIn("/timeline", app)
        self.assertIn("data-resolution-action=\"csr\"", app)
        self.assertIn("data-resolution-action=\"forceOrder\"", app)
        self.assertIn("csrDirectoryOptions", app)
        self.assertIn("Reprocess Email", app)
        ...
        self.assertIn("recordValue", app)
        self.assertIn('key: "customFields.internalRouteCode", label: "Internal Route"', app)
        self.assertNotIn('key: "customerId", label: "Customer"', app)
        self.assertNotIn('key: "id", label: "Record ID"', app)
        ...
        self.assertIn("routingConditionRows", index)
        self.assertIn("addRoutingConditionButton", index)
        self.assertIn("filterConditions", app)
        self.assertIn("filterLogic", app)
        self.assertIn("ROUTING_CONDITION_FIELDS", app)
        self.assertIn("routingRuleNotVisibleAfterSave", app)
        self.assertIn("submittedRuleName", app)
        self.assertIn("nonOrderMoveMode", app)
        self.assertIn("orderStartMoveMode", app)
        self.assertIn("Order start email", index)
        self.assertIn('"knownCustomerNonOrder"', app)
        self.assertIn("Processor Profile", index)
        self.assertIn("Output Profile", index)
        self.assertIn("Power Automate webhook", index)
        self.assertIn("webhookUrl", app)
        self.assertIn("webhookTimeoutSeconds", app)
        self.assertIn("outputFieldChoices", index)
        self.assertIn("processorProfileForm", app)
        self.assertIn("outputProfileForm", app)
        self.assertIn("if (profileId) payload.id = profileId", app)
